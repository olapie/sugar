...
func AssignC(dst any, src any, checker naming.Checker) error {
	if dst == nil {
	...
	func() {
		...
		_ = rtx.JSONCopy(dst, src)
		_ = rtx.GobCopy(dst, src)
	}()

	dv := rtx.IndirectWritableValue(reflect.ValueOf(dst), false)
	...
	return errorx.Wrapf(Validate(dst), "cannot validate")
}

// dst is valid value or pointer to value
func assign(dst reflect.Value, src reflect.Value, nm naming.Checker) error {
	src = rtx.IndirectReadableValue(src)
	dv := rtx.IndirectWritableValue(dst, true)
	switch dv.Kind() {
	case reflect.Bool:
	...
	case reflect.Map:
		...
		return errorx.Wrapf(mapToMap(dv, src, nm), "mapToMap")
	case reflect.Struct:
		return errorx.Wrapf(valueToStruct(dv, src, nm), "valueToStruct")
...
func valueToStruct(dst reflect.Value, src reflect.Value, nm naming.Checker) error {
	switch k := src.Kind(); k {
	case reflect.Map:
		return errorx.Wrapf(mapToStruct(dst, src, nm), "mapToStruct")
	case reflect.Struct:
		return errorx.Wrapf(structToStruct(dst, src, nm), "structToStruct")
	case reflect.String:
		if dst.CanInterface() {
...
func mapToMap(dst reflect.Value, src reflect.Value, nm naming.Checker) error {
	if !src.Type().Key().AssignableTo(dst.Type().Key()) {
		if dst.CanAddr() {
			if addr, ok := dst.Addr().Interface().(json.Unmarshaler); ok {
				...
				return errorx.Wrapf(rtx.JSONCopy(addr, src.Interface()), "json copy")
			}
			...
			if addr, ok := dst.Addr().Interface().(gob.GobDecoder); ok {
				...
				return errorx.Wrapf(rtx.GobCopy(addr, src.Interface()), "gob copy")
			}
		}
...
func mapToStruct(dst reflect.Value, src reflect.Value, nm naming.Checker) error {
...
func structToStruct(dst reflect.Value, src reflect.Value, nm naming.Checker) error {
	for i := 0; i < dst.NumField(); i++ {
		fv := dst.Field(i)
		if fv.IsValid() == false || fv.CanSet() == false {
			continue
...
func SetBytes(target any, b []byte) error {
	...
	v := rtx.IndirectReadableValue(reflect.ValueOf(target))
	if !v.CanSet() {
