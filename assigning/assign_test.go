...
func TestAssignSlice(t *testing.T) {
	...
	values := []any{params}
	var topics []*Topic
	err := assigning.Assign(&topics, values)
...
func TestAssignStruct(t *testing.T) {
	...
	err := assigning.Assign(user, userInfo)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
func TestAssignJSONToStruct(t *testing.T) {
	...
	err := assigning.Assign(i, jsonMap)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
func TestAssignMap(t *testing.T) {
	...
	err := assigning.Assign(&dst, src)
	if err != nil {
		t.Error(err)
	}
	s1, s2 := jsonx.ToString(dst), jsonx.ToString(src)
	if s1 != s2 {
		t.Errorf("mismatch: \n%s\n%s\n", s1, s2)
	}
}
