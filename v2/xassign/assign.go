...
func AssignC(dst any, src any, checker xname.Checker) error {
	if dst == nil {
	...
	func() {
		...
		_ = xruntime.JSONCopy(dst, src)
		_ = xruntime.GobCopy(dst, src)
	}()

	dv := xruntime.IndirectWritableValue(reflect.ValueOf(dst), false)
...
func assign(dst reflect.Value, src reflect.Value, nm xname.Checker) error {
	src = xruntime.IndirectReadableValue(src)
	dv := xruntime.IndirectWritableValue(dst, true)
	switch dv.Kind() {
	case reflect.Bool:
		...
		dv.SetBool(b)
	case reflect.String:
		s, err := conv.ToString(src.Interface())
		if err != nil {
	...
	case reflect.Interface:
		...
		dv.Set(pv.Elem())
	default:
		if xruntime.IsInt(dv) {
			i, err := conv.ToInt64(src.Interface())
			if err != nil {
				return fmt.Errorf("parse int64: %w", err)
			}
			dv.SetInt(i)
		} else if xruntime.IsUint(dv) {
			i, err := conv.ToUint64(src.Interface())
			if err != nil {
				return fmt.Errorf("parse uint64: %w", err)
			}
			dv.SetUint(i)
		} else if xruntime.IsFloat(dv) {
...
func valueToStruct(dst reflect.Value, src reflect.Value, nm xname.Checker) error {
	switch k := src.Kind(); k {
	case reflect.Map:
...
func mapToMap(dst reflect.Value, src reflect.Value, nm xname.Checker) error {
	if !src.Type().Key().AssignableTo(dst.Type().Key()) {
		if dst.CanAddr() {
			if addr, ok := dst.Addr().Interface().(json.Unmarshaler); ok {
				...
				return xerror.Wrapf(xruntime.JSONCopy(addr, src.Interface()), "json copy")
			}
			...
			if addr, ok := dst.Addr().Interface().(gob.GobDecoder); ok {
				...
				return xerror.Wrapf(xruntime.GobCopy(addr, src.Interface()), "gob copy")
			}
		}
...
func mapToStruct(dst reflect.Value, src reflect.Value, nm xname.Checker) error {
...
func structToStruct(dst reflect.Value, src reflect.Value, nm xname.Checker) error {
	for i := 0; i < dst.NumField(); i++ {
		fv := dst.Field(i)
		if fv.IsValid() == false || fv.CanSet() == false {
			continue
		...
		for i := 0; i < src.NumField(); i++ {
			...
			if !xruntime.IsExported(sfName) || !nm.Check(sfName, ft.Name) {
				continue
			}
	...
	for i := 0; i < src.NumField(); i++ {
		sfv := src.Field(i)
		sfName := src.Type().Field(i).Name
		if !sfv.IsValid() || (sfv.CanInterface() && sfv.Interface() == nil) || sfv.IsZero() || !xruntime.IsExported(sfName) {
			continue
		}
...
func Validate(i any) error {
	...
	v = xruntime.IndirectReadableValue(v)
	if v.Kind() == reflect.Struct {
		t := v.Type()
		for j := 0; j < v.NumField(); j++ {
			if !xruntime.IsExported(t.Field(j).Name) {
				continue
			}
...
func SetBytes(target any, b []byte) error {
	...
	v := xruntime.IndirectReadableValue(reflect.ValueOf(target))
	if !v.CanSet() {
		return fmt.Errorf("cannot set value: %T", target)
	}
	if xruntime.IsInt(v) {
		i, err := conv.ToInt64(b)
		if err != nil {
