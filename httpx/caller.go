...
func (c *Caller[IN, OUT]) WithQuery(query url.Values) *Caller[IN, OUT] {
	cc := *c
	var err error
	cc.Endpoint, err = urlx.AppendQuery(c.Endpoint, query)
	if err != nil {
...
func (c *Caller[IN, OUT]) call(ctx context.Context, input IN) (*http.Response, error) {
	...
	if err != nil {
		if err == context.DeadlineExceeded {
			err = errorx.RequestTimeout(err.Error())
		} else {
			if tr, ok := err.(interface{ Timeout() bool }); ok && tr.Timeout() {
				err = errorx.RequestTimeout(err.Error())
			} else {
				err = errorx.Format(600, err.Error())
			}
		}
...
func (c *Caller[IN, OUT]) parseInput(contentType *string, endpoint *string, input any) (io.Reader, error) {
	...
	if v, ok := input.(url.Values); ok {
		newEndpoint, err := urlx.AppendQuery(*endpoint, v)
		if err != nil {
			return nil, err
		}
	...
	newEndpoint, remain := urlx.SetPathParams(*endpoint, input)
	*endpoint = newEndpoint
	...
	kindOfRemain := rtx.IndirectKind(remain)
	switch kindOfRemain {
	...
	default:
		if checking.IsNumber(remain) || checking.IsString(remain) {
