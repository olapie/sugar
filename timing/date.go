...
func (d *Date) ShortText() string {
	if abs(d.t.Sub(time.Now())) <= 2*Day {
		switch {
		case d.IsYesterday():
			return i18.Localize("Yesterday")
		case d.IsToday():
			return i18.Localize("Today")
		case d.IsTomorrow():
			return i18.Localize("Tomorrow")
		}
	}
...
func (d *Date) LongText() string {
	if abs(d.t.Sub(time.Now())) <= 2*Day {
		switch {
		case d.IsYesterday():
			return i18.Localize("Yesterday") + " " + d.PrettyText()
		case d.IsToday():
			return i18.Localize("Today") + " " + d.PrettyText()
		case d.IsTomorrow():
			return i18.Localize("Tomorrow") + " " + d.PrettyText()
		}
	}
	return fmt.Sprintf("%s %s", GetWeekdaySymbol(d.weekday), d.PrettyText())
}
